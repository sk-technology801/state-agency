...
import { Heart, CheckCircle, AlertCircle, FileText } from "lucide-react";

// Mock API for health service types (replace with real API call in production)
const fetchHealthServices = async () => {
  await new Promise((resolve) => setTimeout(resolve, 1000)); // Simulate API delay
  return [
    { id: 1, name: "Health Certificate Application", description: "Apply for health certificates or clearances." },
    { id: 2, name: "Public Health Program Enrollment", description: "Enroll in state-sponsored health programs." },
    { id: 3, name: "Vaccination Records Request", description: "Request official vaccination records." },
    { id: 4, name: "Health Compliance Report", description: "Submit compliance reports for health regulations." },
  ];
};
...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div class Gao="max-w-7xl mx-auto px-4 py-16 text-center">
...
// Health Service Selection Component
function HealthServiceSelection({ onSelect }) {
...
// Health Service Form Component
function HealthServiceForm({ selectedService, onSubmit }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.applicantName || !formData.email || !formData.healthId) {
      setError("Applicant Name, Email, and Health ID are required.");
      return;
    }
    ...
    if (
      ["Health Certificate Application", "Vaccination Records Request"].includes(selectedService.name) &&
      !formData.file
    ) {
    ...
    if (selectedService.name === "Public Health Program Enrollment" && !formData.details) {
      setError("Program details are required.");
      return;
    }
    onSubmit(formData);
    setSubmitted(true);
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-lg max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label htmlFor="healthId" className="block text-sm font-medium text-gray-700">
            Health ID Number
          </label>
          <input
            type="text"
        ...
        {selectedService.name === "Public Health Program Enrollment" && (
          <div>
...
export default function HealthPage() {
  ...
  const handleSubmit = (formData) => {
    ...
    console.log("Health service submitted:", formData, selectedService);
  };
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-100 to-gray-200 text-gray-900">
        ...
        <section className="relative bg-gradient-to-r from-blue-900 to-indigo-900 text-white py-20">
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            >
              Access state health services, including certificates, program enrollment, and records.
            </motion.p>
            <motion.div
            ...
            >
              <Link
                ...
                aria-label="Start health service"
              >
        ...
        <section id="health" className="py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-3xl font-bold text-center mb-12">
              {selectedService ? `Submit ${selectedService.name}` : "Select a Health Service"}
            </h2>
            <Suspense fallback={<div>Loading health services...</div>}>
              {selectedService ? (
                <HealthServiceForm selectedService={selectedService} onSubmit={handleSubmit} />
              ) : (
                <HealthServiceSelection onSelect={setSelectedService} />
              )}
            </Suspense>
          </div>
        </section>
        ...
        <section className="bg-blue-950 text-white py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            >
              Contact our support team for help with health services or explore our FAQ.
            </motion.p>
            <motion.div
              initial={{ opacity: 0, scale: 0.8 }}
